/**
 * FE-TextMaster v5.0 - Professional Text Animation Suite for Adobe After Effects
 * 
 * Powered by FramEmpire | www.framempire.com
 * 
 * ES3 Compliant & Error-Free AE Expression Engine for 100 Presets
 */

(function (thisObj) {

    // ==========================================
    // 10 CATEGORIES & 100 PRESETS
    // ==========================================
    ...
    // ==========================================
    // EXACT 100 PRESETS DATA MAP (EXPRESSION + TARGET PROPERTY)
    // ==========================================

    var PRESET_DATA = {
        // 1. Bounce & Elastic Motion (1-10)
        "01. Heavy Gravity Drop": { target: "position", code: "t = time - inPoint;\nif (t > 0) {\n  gravity = 1500;\n  bounce = 0.6;\n  y = 0.5 * gravity * t * t;\n  value + [0, Math.min(y, 300) * Math.abs(Math.cos(t * 8)) * Math.exp(-t * 3)];\n} else { value; }" },
        "02. Horizontal Wall Snap": { target: "position", code: "freq = 4; decay = 6;\nt = time - inPoint;\nx = 500 * Math.sin(freq * t * 2 * Math.PI) * Math.exp(-decay * t);\nvalue + [x, 0];" },
        "03. Rubber Skew Stretch": { target: "scale", code: "freq = 3; decay = 5;\nt = time - inPoint;\ns = 50 * Math.sin(freq * t * 2 * Math.PI) * Math.exp(-decay * t);\nvalue + [s, -s];" },
        "04. 3D Depth Pendulum": { target: "position", code: "freq = 2; decay = 3;\nt = time - inPoint;\nz = 400 * Math.cos(freq * t * 2 * Math.PI) * Math.exp(-decay * t);\nvalue + [0, 0, z];" },
        "05. Character Stagger Bounce": { target: "position", code: "delay = textIndex * 0.05;\nt = Math.max(0, time - inPoint - delay);\ny = -200 * Math.sin(t * 10) * Math.exp(-t * 5);\nvalue + [0, y];" },
        "06. Overshoot Scale Pulse": { target: "scale", code: "freq = 3.5; decay = 6;\nt = time - inPoint;\ns = 100 * Math.sin(freq * t * 2 * Math.PI) * Math.exp(-decay * t);\nvalue + [s, s];" },
        "07. Baseline Wave Wobble": { target: "position", code: "offset = textIndex * 0.3;\ny = Math.sin((time * 6) - offset) * 30 * Math.exp(-(time - inPoint) * 2);\nvalue + [0, y];" },
        "08. Micro-Jitter Elastic": { target: "position", code: "t = time - inPoint;\nj = (Math.random() - 0.5) * 50 * Math.exp(-t * 8);\nvalue + [j, j];" },
        "09. Reverse Spring Snap": { target: "position", code: "freq = 3; decay = 4;\nt = time - inPoint;\ny = -300 * Math.cos(freq * t * 2 * Math.PI) * Math.exp(-decay * t);\nvalue + [0, y];" },
        "10. Anchor Point Pop Bounce": { target: "scale", code: "t = time - inPoint;\ns = 100 * Math.sin(t * 12) * Math.exp(-t * 4);\nvalue + [s, s];" },

        // 2. Kinetic Typography (11-20)
        "11. Recoil Typewriter": { target: "sourceText", code: "speed = 12;\ntxt = value;\nnumChars = Math.floor((time - inPoint) * speed);\ntxt.substr(0, numChars);" },
        "12. Decaying Wiggle Position": { target: "position", code: "t = time - inPoint;\nw = wiggle(15, 80);\nvalue + (w - value) * Math.exp(-t * 3);" },
        "13. Inertial Kinetic Snap": { target: "position", code: "t = time - inPoint;\ndur = 0.5;\nif (t < dur) {\n  p = t / dur;\n  s = Math.sin(p * Math.PI * 2.5) * Math.exp(-p * 3);\n  value - [500 * (1 - s), 0];\n} else { value; }" },
        "14. Word Pop Cascade": { target: "scale", code: "delay = textIndex * 0.1;\nt = Math.max(0, time - inPoint - delay);\ns = 100 * Math.sin(t * 8) * Math.exp(-t * 4);\nvalue + [s, s];" },
        "15. Incremental Tracking Snap": { target: "tracking", code: "t = time - inPoint;\nvalue + (100 * Math.cos(t * 10) * Math.exp(-t * 5));" },
        "16. Random Character Shuffle": { target: "sourceText", code: "chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';\nt = time - inPoint;\nif (t < 1) {\n  s = '';\n  for(i=0; i<value.length; i++) s += chars[Math.floor(Math.random()*chars.length)];\n  s;\n} else { value; }" },
        "17. Velocity Driven Rotation": { target: "rotation", code: "v = speed;\nvalue + Math.min(v * 0.1, 45) * Math.sin(time * 10);" },
        "18. Kinetic Line Slide": { target: "position", code: "t = time - inPoint;\nx = easeOut(t, 0, 0.4, -600, 0);\ny = Math.sin(t * 12) * 50 * Math.exp(-t * 4);\nvalue + [x, y];" },
        "19. Elastic Tracking Compress": { target: "tracking", code: "t = time - inPoint;\nvalue - (200 * Math.sin(t * 8) * Math.exp(-t * 4));" },
        "20. Chaotic Assembly Snap": { target: "position", code: "seedRandom(textIndex, true);\nrandPos = [random(-400, 400), random(-400, 400)];\nt = Math.max(0, time - inPoint - (textIndex * 0.03));\nvalue + (randPos * Math.exp(-t * 5));" },

        // 3. Glitch & Digital (21-30)
        "21. RGB Channel Split": { target: "position", code: "posterizeTime(10);\nvalue + [(Math.random()-0.5)*30, 0];" },
        "22. Binary Code Matrix Reveal": { target: "sourceText", code: "t = time - inPoint;\ndur = 1.5;\nprogress = Math.min(1, t / dur);\nlen = Math.floor(progress * value.length);\nres = value.substr(0, len);\nfor(i=len; i<value.length; i++) res += Math.floor(Math.random()*2);\nres;" },
        "23. Signal Interference Flicker": { target: "opacity", code: "posterizeTime(15);\n(Math.random() > 0.3) ? 100 : 20;" },
        "24. Digital Jitter Position": { target: "position", code: "posterizeTime(12);\nt = time - inPoint;\n(t < 1) ? value + [(Math.random()-0.5)*60, (Math.random()-0.5)*20] : value;" },
        "25. Pixelated Snap Scale": { target: "scale", code: "posterizeTime(8);\nt = time - inPoint;\ns = (t < 0.5) ? Math.floor(Math.random()*4)*30 : 100;\n[s, s];" },
        "26. Corrupted Hex Glitch": { target: "sourceText", code: "hex = '0123456789ABCDEF';\nt = time - inPoint;\nif (t < 1) {\n  res = '';\n  for(i=0; i<value.length; i++) res += hex[Math.floor(Math.random()*16)];\n  res;\n} else { value; }" },
        "27. Horizontal Slice Offset": { target: "position", code: "posterizeTime(12);\nx = (textIndex % 2 == 0 ? 1 : -1) * 80 * Math.exp(-(time-inPoint)*4);\nvalue + [x, 0];" },
        "28. VCR Tracking Jump": { target: "position", code: "t = time - inPoint;\ny = (Math.sin(t * 20) > 0.8) ? 40 : 0;\nvalue + [0, y];" },
        "29. Static Noise Opacity Pop": { target: "opacity", code: "seedRandom(index + time*100, false);\n(time - inPoint < 0.8) ? random(0, 100) : 100;" },
        "30. Glitch Rotation Snap": { target: "rotation", code: "posterizeTime(10);\nt = time - inPoint;\n(t < 0.6) ? (Math.random()-0.5)*90 : 0;" },

        // 4. UI & HUD FX (31-40)
        "31. Cursor Blink Typewriter": { target: "sourceText", code: "speed = 10;\nnumChars = Math.floor((time - inPoint) * speed);\ncursor = (Math.floor(time * 3) % 2 == 0) ? '|' : '';\nvalue.substr(0, numChars) + cursor;" },
        "32. Digital Counter Accent": { target: "sourceText", code: "val = Math.floor(easeOut(time - inPoint, 0, 2, 0, 100));\nval + '%';" },
        "33. HUD Bracket Target Lock": { target: "scale", code: "t = time - inPoint;\ns = 200 * Math.exp(-t * 6);\nvalue + [s, s];" },
        "34. Terminal Command Line Pop": { target: "position", code: "t = time - inPoint;\nstep = Math.floor(t * 8);\nvalue + [0, -step * 5 * Math.exp(-t * 2)];" },
        "35. Coordinate Position Lock": { target: "sourceText", code: "x = Math.floor(position[0] + wiggle(5, 20)[0]);\ny = Math.floor(position[1] + wiggle(5, 20)[1]);\n'X:' + x + ' Y:' + y;" },
        "36. Hexadecimal Morph": { target: "sourceText", code: "t = time - inPoint;\nif(t < 0.8) {\n  '0x' + Math.floor(Math.random()*65535).toString(16).toUpperCase();\n} else { value; }" },
        "37. Cyberpunk Neon Strike": { target: "opacity", code: "t = time - inPoint;\n(t < 0.5) ? Math.sin(t * 40) * 100 : 100;" },
        "38. Loading Status Pulse": { target: "opacity", code: "50 + Math.sin((time - inPoint) * 10) * 50;" },
        "39. Digital Clock Tick Snap": { target: "rotation", code: "step = Math.floor((time - inPoint) * 4);\nstep * 90;" },
        "40. Radar Sweep Tracking": { target: "tracking", code: "Math.sin(time * 5) * 50;" },

        // 5. Physics, Wave & Fluid (41-50)
        "41. Sinusoidal Position Wave": { target: "position", code: "y = Math.sin((time * 5) + (textIndex * 0.5)) * 40;\nvalue + [0, y];" },
        "42. Jelly Body Distortion": { target: "scale", code: "t = time - inPoint;\nsx = 100 + 40 * Math.sin(t * 10) * Math.exp(-t * 3);\nsy = 100 - 40 * Math.sin(t * 10) * Math.exp(-t * 3);\n[sx, sy];" },
        "43. Fluid Ripple Offset": { target: "position", code: "t = time - inPoint;\nr = Math.sin(t * 8 - textIndex * 0.4) * 30 * Math.exp(-t * 2);\nvalue + [0, r];" },
        "44. Per-Character Drop Bounce": { target: "position", code: "t = Math.max(0, time - inPoint - textIndex * 0.08);\ny = Math.abs(Math.sin(t * 6)) * -150 * Math.exp(-t * 3);\nvalue + [0, -y];" },
        "45. Pendulum Swing Wave": { target: "rotation", code: "t = time - inPoint;\nvalue + 45 * Math.cos(t * 4) * Math.exp(-t * 1.5);" },
        "46. Floating Magnet Attract": { target: "position", code: "t = time - inPoint;\nd = 300 * Math.exp(-t * 4);\nvalue + [Math.cos(t*10)*d, Math.sin(t*10)*d];" },
        "47. Wind Blow Wobble": { target: "rotation", code: "value + Math.sin(time * 8) * 15 * Math.exp(-(time-inPoint)*2);" },
        "48. Zero Gravity Float": { target: "position", code: "x = Math.sin(time * 1.5) * 20;\ny = Math.cos(time * 2) * 25;\nvalue + [x, y];" },
        "49. Bubble Pop Expansion": { target: "scale", code: "t = time - inPoint;\ns = (t < 0.2) ? easeOut(t, 0, 0.2, 0, 130) : easeOut(t, 0.2, 0.4, 130, 100);\n[s, s];" },
        "50. String Elastic Tension": { target: "position", code: "t = time - inPoint;\ny = -200 * Math.exp(-t * 2) * Math.cos(t * 15);\nvalue + [0, y];" },

        // 6. 3D Space & Isometric (51-60)
        "51. 3D Flip Bounce Reveal": { target: "rotation", code: "t = Math.max(0, time - inPoint - textIndex * 0.05);\n180 * Math.cos(t * 6) * Math.exp(-t * 3);" },
        "52. Isometric Slide Snap": { target: "position", code: "t = time - inPoint;\noffset = 400 * Math.exp(-t * 5) * Math.cos(t * 10);\nvalue + [offset, offset * 0.5];" },
        "53. 3D Helix Rotation": { target: "rotation", code: "t = time - inPoint;\nMath.sin(t * 5 + textIndex * 0.5) * 90 * Math.exp(-t * 2);" },
        "54. Z-Space Elastic Drop": { target: "position", code: "t = time - inPoint;\nz = -1000 * Math.cos(t * 5) * Math.exp(-t * 3);\nvalue + [0, 0, z];" },
        "55. Perspective Tilt Wobble": { target: "rotation", code: "t = time - inPoint;\n60 * Math.sin(t * 6) * Math.exp(-t * 2.5);" },
        "56. 3D Box Unfold": { target: "rotation", code: "t = Math.max(0, time - inPoint - textIndex * 0.1);\neaseOut(t, 0, 0.5, -90, 0);" },
        "57. Camera Focal Snap": { target: "scale", code: "t = time - inPoint;\ns = 400 * Math.exp(-t * 6);\nvalue + [s, s];" },
        "58. 3D Cubical Spin": { target: "rotation", code: "t = time - inPoint;\n(t < 0.8) ? easeOut(t, 0, 0.8, 360, 0) : 0;" },
        "59. 3D Extrusion Depth Pop": { target: "position", code: "t = time - inPoint;\nz = 500 * Math.sin(t * 8) * Math.exp(-t * 4);\nvalue + [0, 0, z];" },
        "60. Depth Stagger Push": { target: "position", code: "t = Math.max(0, time - inPoint - textIndex * 0.06);\nz = -500 * Math.exp(-t * 4);\nvalue + [0, 0, z];" },

        // 7. Particle & Shatter (61-70)
        "61. Gravity Disintegration": { target: "position", code: "t = Math.max(0, time - inPoint - textIndex * 0.05);\ny = 0.5 * 1000 * t * t;\nvalue + [0, y];" },
        "62. Reverse Shatter Re-assembly": { target: "position", code: "seedRandom(textIndex, true);\nstartPos = [random(-600,600), random(-600,600)];\nt = time - inPoint;\ndur = 0.8;\nvalue + easeOut(t, 0, dur, startPos, [0,0]);" },
        "63. Explosive Particle Outward": { target: "position", code: "seedRandom(textIndex, true);\ndir = [random(-1,1), random(-1,1)];\nt = time - inPoint;\nvalue + (dir * 500 * Math.sin(t * 5) * Math.exp(-t * 3));" },
        "64. Implosive Snap Reveal": { target: "position", code: "seedRandom(textIndex, true);\nstartPos = [random(-400,400), random(-400,400)];\nt = time - inPoint;\nvalue + (startPos * Math.exp(-t * 6));" },
        "65. Slice Jump Unify": { target: "position", code: "y = (textIndex % 2 == 0 ? -200 : 200) * Math.exp(-(time-inPoint)*5);\nvalue + [0, y];" },
        "66. Asymmetrical Fragment Pop": { target: "position", code: "seedRandom(textIndex, true);\nvalue + [random(-100,100), random(-200,200)] * Math.exp(-(time-inPoint)*4);" },
        "67. Magnetic Attract Assembly": { target: "position", code: "t = time - inPoint;\nr = 400 * Math.exp(-t * 5);\nvalue + [Math.cos(textIndex)*r, Math.sin(textIndex)*r];" },
        "68. Impact Shockwave Offset": { target: "position", code: "t = time - inPoint;\nshk = Math.sin(t * 30) * 40 * Math.exp(-t * 6);\nvalue + [shk, shk];" },
        "69. Sand Dissolve Rebound": { target: "position", code: "seedRandom(textIndex, true);\nt = time - inPoint;\nvalue + [random(-50,50), random(0,200)] * Math.exp(-t * 3);" },
        "70. Pixel Collapse Re-bound": { target: "scale", code: "t = time - inPoint;\ns = (t < 0.3) ? 10 : 100 + 50 * Math.exp(-t*5);\n[s, s];" },

        // 8. Gaming & Pop Style (71-80)
        "71. Score Elastic Popup": { target: "scale", code: "t = time - inPoint;\ns = 100 + 150 * Math.sin(t * 10) * Math.exp(-t * 5);\n[s, s];" },
        "72. Damage Number Float": { target: "position", code: "t = time - inPoint;\nvalue - [0, easeOut(t, 0, 0.8, 0, 150)];" },
        "73. Comic Book POW Pop": { target: "scale", code: "t = time - inPoint;\ns = (t < 0.1) ? easeOut(t,0,0.1,0,160) : easeOut(t,0.1,0.4,160,100);\n[s, s];" },
        "74. Subscribe Button Shake": { target: "rotation", code: "t = time - inPoint;\nMath.sin(t * 20) * 15 * Math.exp(-t * 4);" },
        "75. Level Up Banner Bounce": { target: "position", code: "t = time - inPoint;\ny = -300 * Math.sin(t * 8) * Math.exp(-t * 4);\nvalue + [0, y];" },
        "76. Emoji Pulse Bump": { target: "scale", code: "100 + Math.abs(Math.sin((time - inPoint) * 8)) * 30;" },
        "77. Speech Bubble Snap": { target: "scale", code: "t = time - inPoint;\nsx = easeOut(t, 0, 0.3, 0, 100);\nsy = easeOut(t, 0, 0.3, 0, 100) + 30 * Math.sin(t*15)*Math.exp(-t*5);\n[sx, sy];" },
        "78. Arcade Combo Bounce": { target: "scale", code: "t = time - inPoint;\ns = 100 + Math.abs(Math.cos(t * 12)) * 40 * Math.exp(-t * 3);\n[s, s];" },
        "79. Retro 8-Bit Jump": { target: "position", code: "step = Math.floor((time - inPoint) * 12);\ny = -Math.abs(Math.sin(step * 0.5)) * 80;\nvalue + [0, y];" },
        "80. Coin Collect Rise": { target: "position", code: "t = time - inPoint;\ny = -200 * (1 - Math.exp(-t * 4));\nvalue + [0, y];" },

        // 9. Mathematical Procedures (81-90)
        "81. Harmonic Oscillator Physics": { target: "position", code: "m = 1; k = 80; c = 8;\nt = time - inPoint;\nw = Math.sqrt(k/m);\ndisp = Math.exp(-c*t) * Math.cos(w*t);\nvalue + [0, -200 * disp];" },
        "82. Fibonacci Scale Stagger": { target: "scale", code: "fib = [1, 1, 2, 3, 5, 8, 13, 21];\nidx = Math.min(Math.floor((time - inPoint) * 10), fib.length - 1);\ns = fib[idx] * 10;\n[s, s];" },
        "83. Perlin Noise Position Shift": { target: "position", code: "nx = noise(time * 2) * 100;\nny = noise((time + 10) * 2) * 100;\nvalue + [nx, ny];" },
        "84. Exponential Decay Bounce": { target: "position", code: "t = time - inPoint;\ny = -300 * Math.pow(0.5, t * 4) * Math.abs(Math.cos(t * 10));\nvalue + [0, y];" },
        "85. Logarithmic Tracking Snap": { target: "tracking", code: "t = Math.max(0.01, time - inPoint);\nvalue + (200 / Math.log(t * 10 + 1));" },
        "86. Modulo Index Offset Wave": { target: "position", code: "y = (textIndex % 3 == 0) ? Math.sin(time * 6) * 50 : 0;\nvalue + [0, y];" },
        "87. Trigonometric Swing": { target: "rotation", code: "Math.tan(Math.sin(time * 3)) * 20;" },
        "88. Random Seed Frequency Pop": { target: "scale", code: "seedRandom(Math.floor(time * 4), true);\ns = random(80, 120);\n[s, s];" },
        "89. Velocity Driven Elasticity": { target: "scale", code: "v = length(velocity);\ns = v * 0.05;\n[100 + s, 100 - s];" },
        "90. Fractal Noise Displacement": { target: "position", code: "try { l = layer('Null 1'); n = 0; } catch(e) { n = Math.sin(time*10)*20; }\nvalue + [n, n];" },

        // 10. Minimal & Clean (91-100)
        "91. Smooth Overshoot Slide": { target: "position", code: "t = time - inPoint;\nx = easeOut(t, 0, 0.6, -400, 0) + 30 * Math.sin(t * 8) * Math.exp(-t * 4);\nvalue + [x, 0];" },
        "92. Subtle Anchor Shift": { target: "position", code: "t = time - inPoint;\nvalue + [0, 50 * Math.exp(-t * 5)];" },
        "93. Minimal Vertical Drop": { target: "position", code: "t = time - inPoint;\ny = easeOut(t, 0, 0.5, -100, 0);\nvalue + [0, y];" },
        "94. Line Mask Unfold Elastic": { target: "scale", code: "t = time - inPoint;\nsx = easeOut(t, 0, 0.4, 0, 100) + 20 * Math.sin(t * 10) * Math.exp(-t * 5);\n[sx, 100];" },
        "95. Soft Tracking Expansion": { target: "tracking", code: "t = time - inPoint;\neaseOut(t, 0, 1, 100, 0);" },
        "96. Dual-Direction Snap": { target: "position", code: "dir = (textIndex % 2 == 0) ? -1 : 1;\nt = time - inPoint;\nvalue + [dir * 200 * Math.exp(-t * 5), 0];" },
        "97. Opacity Pulse Bounce": { target: "opacity", code: "t = time - inPoint;\neaseOut(t, 0, 0.3, 0, 100) + 30 * Math.sin(t * 12) * Math.exp(-t * 4);" },
        "98. Clean Left Cascade": { target: "position", code: "t = Math.max(0, time - inPoint - textIndex * 0.04);\nx = -150 * Math.exp(-t * 6);\nvalue + [x, 0];" },
        "99. Subtle Rotation Settle": { target: "rotation", code: "t = time - inPoint;\n15 * Math.cos(t * 5) * Math.exp(-t * 3);" },
        "100. Elegant Rise Snap": { target: "position", code: "t = time - inPoint;\ny = 120 * Math.exp(-t * 4) * Math.cos(t * 6);\nvalue + [0, y];" }
    };
    ...
    function applyPresetToSelectedLayers(presetName, statusText) {
        ...
        var presetInfo = PRESET_DATA[presetName];
        if (!presetInfo) {
            alert("Could not find preset data for '" + presetName + "'.", "FE-TextMaster");
            return;
        }
        ...
        try {
            var count = 0;
            var targetType = presetInfo.target;
            var exprCode = presetInfo.code;
            ...
            var msg = "Applied '" + presetName + "' to " + count + " Text Layer(s).";
    ...
    function buildUI(thisObj) {
        ...
        var subText = headerGroup.add("statictext", undefined, "100 AE Motion Expressions & Toolkit");
        ...
        // 2. LIVE TEXT PREVIEW DISPLAY BOX
        var previewBox = tabPresets.add("panel", undefined, "Live Text Animation Preview");
        ...
        // SAFE SCRIPTUI EVENT-DRIVEN ANIMATION UPDATE
        win.onIdle = function () {
            try {
                if (!animState || !txtPreviewDisplay) return;
                var sample = "FramEmpire- A Revolution of Animation";
                var now = new Date().getTime();
                var t = ((now - animState.startTime) % 2000) / 1000;
                var name = animState.activePreset || "";
                ...
                if (name.indexOf("Typewriter") !== -1 || name.indexOf("Recoil") !== -1 || name.indexOf("Cursor") !== -1) {
                    ...
                    txtPreviewDisplay.text = (cnt < sample.length) ? sample.substring(0, cnt) + "|" : sample + cur;
                } else if (name.indexOf("Binary") !== -1 || name.indexOf("Matrix") !== -1 || name.indexOf("Hex") !== -1) {
                    ...
                    txtPreviewDisplay.text = outStr;
                } else if (name.indexOf("Wiggle") !== -1 || name.indexOf("Jitter") !== -1 || name.indexOf("Noise") !== -1) {
                    txtPreviewDisplay.text = (Math.random() > 0.5 ? "  " : " ") + sample;
                } else if (name.indexOf("Counter Accent") !== -1) {
                    var val = Math.floor(Math.min(t / 1.5, 1) * 100);
                    txtPreviewDisplay.text = "FramEmpire Loading: " + val + "%";
                } else if (name.indexOf("Coordinate Position") !== -1) {
                    txtPreviewDisplay.text = "X:" + Math.floor(200 + Math.random()*20) + " Y:" + Math.floor(400 + Math.random()*20);
                } else {
                    txtPreviewDisplay.text = sample;
                }
            } catch(eIdle) {}
        };
